...
SUPPORTED_FIELDS: List[FieldSpec] = [
    ...
    FieldSpec("AUTO_TRANSPOSE", "自动适配音域", "bool", "演奏与切区", "自动把整首歌整体挪到更适合当前可弹范围的位置。大多数情况建议开启。"),
    FieldSpec("AUTO_SHIFT_FROM_RANGE", "按音域自动判断右移窗口", "bool", "演奏与切区", "当你的可弹范围已经完整落在基础窗口里时，会自动关闭右移窗口，避免无意义切换。"),
    FieldSpec("USE_SHIFT_OCTAVE", "启用右移窗口", "bool", "演奏与切区", "允许使用右移一组窗口来扩展可覆盖的音域。短区间且已完全覆盖时通常会自动关闭。"),
    FieldSpec("SHIFT_KEY", "右移窗口按键", "text", "演奏与切区", "切换到右移窗口时要按的键。默认是 Shift。"),
    ...
    FieldSpec("SHIFT_WEIGHT", "右移窗口偏好", "float", "演奏与切区", "越大越倾向选择右移窗口。短区间固定窗口时这个值会自动降权。"),
